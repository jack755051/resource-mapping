    ...
    async (language: string, { rejectWithValue }) => {
        try {
            ...
            if (results[0].status === 'fulfilled') {
                ...
                console.warn('[Redux System] SupportCategories API Failed, using Mock Data.', results[0].reason);
                supportCategories = SupportMapper.toDomainCategoryList(MOCK_SUPPORT_CATEGORIES);
            }
            ...
            if (results[1].status === 'fulfilled') {
                ...
                console.warn('[Redux System] ProductCategories API Failed, using empty array.', results[1].reason);
                productCategories = [];
            }
            ...
            if (results[2].status === 'fulfilled') {
                ...
                console.warn('[Redux System] Locations API Failed, using Mock Data.', results[2].reason);
                locationsCategories = ContactMapper.toDomainList(MOCK_CONTACT_API_RESPONSE);
            }
            ...
            return rejectWithValue(error.message || 'Failed to fetch system resources');
        }
    }
);
...
export const selectSupportCategories = (state: { system: ExtendedSystemState }) =>
    state.system.supportCategories;

export const selectProductCategories = (state: { system: ExtendedSystemState }) =>
    state.system.productCategories;

export const selectLocationsCategories = (state: { system: ExtendedSystemState }) =>
...
// 组合 selector - 获取所有资源
export const selectSystemResources = (state: { system: ExtendedSystemState }) => ({
