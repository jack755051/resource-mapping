...
import {
  ...
  SupportListResDto,
} from '../response/support.response';
...
export const SupportService = {
  /** 取得支援分類 */
  handleGetSupportCategories: async (
    lang: string
  ): Promise<SupportCategory[]> => {
    // ✅ apiClient 已自動解包 APIResponse，直接獲得 data 內容
    const categories = await apiClient<SupportCategoryResDto[]>(
      CommonUrl.CONSTANTS_SUPPORT_CATEGORIES,
      {
        method: 'GET',
        headers: {
  ...
  /** 取得支援資源列表 (含防呆保護) */
  handleGetSupportList: async (
    payload: SupportReqDto,
    lang: string
  ): Promise<PaginatedSupportResource> => {
    try {
      // ✅ apiClient 已自動解包 APIResponse，返回 { items, meta }
      const res = await apiClient<PaginatedResDto<SupportListResDto>>(
        CommonUrl.SUPPORT_RESOURCES,
        {
          params: payload,
          headers: {
