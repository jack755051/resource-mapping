...
import { Search, FileText, Download, HelpCircle, ChevronRight, FileCode, Wrench } from 'lucide-react';
...
// 1. 模擬資料：支援項目
const resources = [
    { id: 1, title: 'GC-IP50 系列 - 使用者操作手冊', category: 'manual', date: '2025-10-15', type: 'PDF', size: '2.5 MB' },
    { id: 2, title: 'Smart Eye X1 韌體更新 v2.0.4', category: 'firmware', date: '2026-01-10', type: 'ZIP', size: '45 MB' },
    { id: 3, title: '如何設定 RTSP 串流？', category: 'faq', date: '2025-12-05', type: 'Article', size: '' },
    { id: 4, title: 'CMS 電腦版監控軟體 (Windows)', category: 'software', date: '2025-11-20', type: 'EXE', size: '120 MB' },
    { id: 5, title: '防水攝影機安裝注意事項', category: 'faq', date: '2025-09-01', type: 'Article', size: '' },
    { id: 6, title: '5MP 系列鏡頭規格書', category: 'manual', date: '2025-08-30', type: 'PDF', size: '1.2 MB' },
    { id: 7, title: '忘記 NVR 管理員密碼怎麼辦？', category: 'faq', date: '2025-12-22', type: 'Article', size: '' },
    { id: 8, title: 'GC-30450 快速安裝指南', category: 'manual', date: '2025-07-11', type: 'PDF', size: '0.8 MB' },
    // ... 更多資料以測試分頁
];
...
const getIcon = (type: string, category: string) => {
    if (category === 'firmware' || category === 'software') return <FileCode className="w-5 h-5 text-blue-600" />;
    if (category === 'faq') return <HelpCircle className="w-5 h-5 text-orange-500" />;
    if (category === 'manual') return <FileText className="w-5 h-5 text-primary" />;
                        ...
                        >
                            <div className="relative group">
                                ...
                                <div className="relative flex items-center bg-background rounded-full border border-border/50 shadow-lg overflow-hidden">
                                    ...
                                    />
                                    <Button className="mr-1 rounded-full px-6" size="lg">搜尋</Button>
                                </div>
                            </div>
                        </motion.div>
                    </div>
                    ...
                    <AnimatePresence mode="popLayout">
                        {currentData.length > 0 ? (
                            currentData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <div className="shrink-0 flex items-center gap-3">
                                        ...
                                        <Button variant="ghost" size="icon" className="rounded-full text-muted-foreground group-hover:text-primary group-hover:bg-primary/10">
                                            {item.category === 'faq' ? <ChevronRight className="w-5 h-5" /> : <Download className="w-5 h-5" />}
                                        </Button>
                                    </div>

                                    {/* 隱形連結 */}
                                    <a href="#" className="absolute inset-0" aria-label="View Item"></a>
                                </motion.div>
                            ))
                        ) : (
                            <div className="flex flex-col items-center justify-center py-20 text-center">
                                ...
                                <p className="text-muted-foreground">請嘗試其他關鍵字或切換分類。</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
